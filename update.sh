...
if [ "$(docker ps -q -f name=$custom_container_name)" ]; then
    ...
    echo "Stopping existing container with name $custom_container_name..."
...
docker run --name $custom_container_name -v /home/docker/fika/user/profiles:/usr/src/app/profiles -p 3034:3034 tarkov_tracker
